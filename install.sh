...
# Defaults
APPNAME="rancher"
APPDIR="$HOME/.local/share/srv/docker/rancher"
DATADIR="$HOME/.local/share/srv/docker/rancher/files"
INSTDIR="$HOME/.local/share/dockermgr/rancher"
REPO="${DOCKERMGRREPO:-https://github.com/dockermgr}/rancher"
REPORAW="$REPO/raw/$REPO_BRANCH"
APPVERSION="$(__appversion "$REPORAW/version.txt")"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Setup plugins
HUB_URL="rancher/rancher:latest"
...
if docker ps -a | grep -qs "$APPNAME"; then
  ...
  [[ -n "$SERVER_PORT" ]] && printf_blue "and should be available at: http://$SERVER_LISTEN:$SERVER_PORT_OTHER or http://$SERVER_HOST:$SERVER_PORT_OTHER"
  [[ -n "$SERVER_PORT" ]] && printf_red "If you can not get password then run: dockermgr exec $APPNAME reset-password"
